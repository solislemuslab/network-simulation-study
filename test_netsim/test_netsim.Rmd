...
The package builds on the approach of Stadler for `TreeSim`, but with modifications by Zhang et al. (2018) for including hybridization as part of the BD stochastic process. The functions are named similar to those of `TreeSim` but with `bdh` insted of just `bd`. 
...
- Hybridization probability as a function of genetic distance (the more distant two lineages the easier for them to coalesce?)
- Hybridization probability covariating with a trait along the phylogeny (phylo-comparative-methods-like?)
...
First let's simulate the same tree samples using `TreeSim` as a control for comparing the resuts by `NetSim`:
...
The plot engine `NetSim::plottable.net` is said to reorient the tree ofr improved plotting with respect to hybrid edges. However, this also creates extinct tips for hybridization events (see the plot on the right below, sister to `t11` and compare with the same node in the plot to the left). 
