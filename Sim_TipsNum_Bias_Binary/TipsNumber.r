...
#TreeSim age=4
yuleTS <- sim.bd.age(age = 4, numbsim = numbsim, lambda = 1, mu = 0, frac = 1, mrca = FALSE,complete = TRUE, K = 0)
...
par(mfrow=c(2,2))
h <- hist(TipYulTS4,main="Tip Number from Yule simulations TreeSim\n( numSimb=500, age=4, lambda=1)",xlab="Tips")
text(x=quantile(h$breaks,0.7),y=quantile(h$counts,0.7),labels=paste("Tips>=10: ",PorTip10YuTS4," %"),cex=1.3)

h <- hist(TipYulNS4,main="Tip Number from Yule simulations NetSim\n( numSimb=500, age=4, lambda=1)",xlab="Tips")
text(x=quantile(h$breaks,0.7),y=quantile(h$counts,0.7),labels=paste("Tips>=10: ",PorTip10YuNS4," %"),cex=1.3)

h <- hist(TipYulTS7,main="Tip Number from Yule simulations TreeSim\n( numSimb=500, age=6, lambda=1)",xlab="Tips")
text(x=quantile(h$breaks,0.7),y=quantile(h$counts,0.7),labels=paste("Tips>=10: ",PorTip10YuTS7," %"),cex=1.3)

h <- hist(TipYulNS7,main="Tip Number from Yule simulations NetSim\n( numSimb=500, age=6, lambda=1)",xlab="Tips")
text(x=quantile(h$breaks,0.7),y=quantile(h$counts,0.7),labels=paste("Tips>=10: ",PorTip10YuNS7," %"),cex=1.3)
