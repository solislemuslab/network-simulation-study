...
for(i in 1:100){
numbsim=100
bdNS <- NetSim::sim.bdh.taxa.ssa(n=15, numbsim = numbsim, lambda = 0.9, mu = 0.5, nu = 0,hybprops = c(0.5, 0.5, 0.5), hyb.inher.fxn = NetSim::make.beta.draw(1, 1), frac = 0.9,mrca = TRUE, complete = FALSE, stochsampling = FALSE, hyb.rate.fxn = NULL, trait.model = NULL)
...
for(i in 1:100){ 
numbsim = 100
bdTS <- sim.bd.taxa(n = 15, numbsim = numbsim, lambda = 0.9, mu = 0.5, frac = 0.9, complete = FALSE,stochsampling = TRUE)
## remove any trees with no taxa
...
par(mfrow=c(1,2))
hist(BiasLambdaTSn15,main="Bias of Lambda from Birth-death TreeSim\n(100 numbsim repeated 100 times)\n(n=15, lambda=0.9)",xlab=expression(paste(lambda, " bias")))
abline(v=0)
hist(BiasLambdaNSn15,main="Bias of Lambda from Birth-death NetSim\n(100 numbsim repeated 100 times)\n(n=15, lambda=0.9)",xlab=expression(paste(lambda, " bias")))
abline(v=0)
#dev.off()