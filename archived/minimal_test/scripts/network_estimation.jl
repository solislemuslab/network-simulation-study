...
######### code from Claudia's tutorial
cfs = readTableCF("../data/1_seqgen.CFs.csv")
start_tree = readTopology("../data/1_seqgen.QMC.tre")

# calculate a h=0 network
net0 = snaq!(start_tree, cfs, hmax=0, filename="../data/snaq/snaq_minimal_h0", seed=1234, runs = nruns)

# calculate a h=1 network
net1 = snaq!(net0, cfs, hmax=1, filename="../data/snaq/snaq_minimal_h1", seed=1324, runs = nruns)

# calculate a h=2 network
net2 = snaq!(net1, cfs, hmax=2, filename="../data/snaq/snaq_minimal_h2", seed=1432, runs = nruns)

# calculate a h=3 network
net3 = snaq!(net2, cfs, hmax=3, filename="../data/snaq/snaq_minimal_h3", seed=1432, runs = nruns)

# calculate a h=4 network
net4 = snaq!(net3, cfs, hmax=4, filename="../data/snaq/snaq_minimal_h4", seed=2134, runs = nruns)

# calculate a h=5 network
net5 = snaq!(net4, cfs, hmax=5, filename="../data/snaq/snaq_minimal_h5", seed=2143, runs = nruns)
