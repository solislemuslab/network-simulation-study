---
title: "Networks characterization from Birth Dead method with SiPhyNetwork"
author: 
    - Carlos Acosta
output: 
...
Simulation of 300 Trees with the following parameters using SiPhyNetwork:
...
numbsim=300
n1<-15
bdNS <- SiPhyNetwork::sim.bdh.taxa.ssa(n=n1, 
                                 numbsim = numbsim, 
                                 lambda = 0.9, 
                                 mu = 0, 
                                 nu = 0.01,
...
bdNS <- bdNS[sapply(X = bdNS, FUN = is.phylo)]#birth dead tree with SiPhyNetwork with value 0 or 1
...
res<-NetDescription(bdNS)
res2<-data.frame(HybTab[i,],res,row.names = NULL)
outputNu0.04<-rbind(outputNu0.04,res2)
}
...
outputNu0.04$CombHybP<-apply(outputNu0.04[,c("Gen", "Deg", "Neu")],1,paste0,collapse="-")
...
outputNu0.04[,"CombHybP"]<-factor(outputNu0.04[,"CombHybP"],levels=LevChar)
#########################################
head(outputNu0.02)
head(outputNu0.01)
head(outputNu0.04)
...
## Identifiable
```{r chfwufnk55.4, cache = TRUE, fig.asp = 1.0, dev.args  = list(pointsize = 10)}
########
####
out<-c()
for(i in 1:length(bdNS)){
res<-SibCross(Tree2=bdNS[[i]])$infor
ro<-any(res%in%"Identificable")
out<-c(out,ro)
}


tree<-bdNS[[which(out)[1]]]
res<-SibCross(Tree2=tree);res
plot(tree)
nod<-res$ret[res$infor=="Identificable",]
nodelabels(node=nod)

```


## Not Identifiable (sibling cross)
```{r chfwufdgfnk55.4, cache = TRUE, fig.asp = 1.0, dev.args  = list(pointsize = 10)}
########
####
out<-c()
for(i in 1:length(bdNS)){
res<-SibCross(Tree2=bdNS[[i]])$infor
ro<-any(res%in%"No-Identificable")
out<-c(out,ro)
}
...
res<-SibCross(Tree2=tree);res
plot(tree)
nod<-res$ret[res$infor=="No-Identificable",]
nodelabels(node=nod)
...
## No level One
```{r chufnk55.4, cache = TRUE, fig.asp = 1.0, dev.args  = list(pointsize = 10)}
########
####
out<-c()
for(i in 1:length(bdNS)){
...
res<-SibCross(Tree2=tree);res
plot(tree)
nod<-res$ret[res$infor=="RetCiclicI",]
nodelabels(node=nod)
```
...
tabbNu0.04<-table(outputNu0.04[,c("RetNum","CombHybP")])


xxlab1<-"hybprops combination                              "
pop<-par(mfrow=c(1,3),oma = c(0, 0, 2, 0))
barplot(tabbNu0.01,legend.text=rownames(tabbNu0.01),col=colors1,xlim=c(0,12),main="nu=0.01",xlab=xxlab1,las=2)
barplot(tabbNu0.02,legend.text=rownames(tabbNu0.02),col=colors1,xlim=c(0,12),main="nu=0.02",xlab=xxlab1,las=2)
barplot(tabbNu0.04,legend.text=rownames(tabbNu0.04),col=colors1,xlim=c(0,12),main="nu=0.04",xlab=xxlab1,las=2)
par(pop)
...
vioplot(TimeTree~CombHybP,data=outputNu0.04,main="0.04",ylim=c(0,16),las=2)
par(pop)
...
vioplot(NodeNum~CombHybP,data=outputNu0.04,main="nu = 0.04",ylim=c(14,33),las=2)
par(pop)
...
SuboutputNu0.04Ret<-subset(outputNu0.04,RetNum>0)
tabwNu0.04Ret<-table(SuboutputNu0.04Ret[,c("NotIdent","CombHybP")])

tabw1Nu0.04Ret<-tabwNu0.04Ret
retTotNu0.04Ret<-apply(tabw1Nu0.04Ret,2,sum)

for(i in 1:ncol(tabw1Nu0.04Ret)){
tabw1Nu0.04Ret[,i]<-tabw1Nu0.04Ret[,i]/retTotNu0.04Ret[i]
}



pop<-par(mfrow=c(1,3),oma = c(0, 0, 2, 0))
barplot(tabw1Nu0.01Ret,legend.text=rownames(tabw1Nu0.01Ret),col=colors1,main="nu=0.01",xlim=c(0,12),xlab=xxlab1,las=2)
barplot(tabw1Nu0.02Ret,legend.text=rownames(tabw1Nu0.02Ret),col=colors1,main="nu=0.02",xlim=c(0,12),xlab=xxlab1,las=2)

barplot(tabw1Nu0.04Ret,legend.text=rownames(tabw1Nu0.04Ret),col=colors1,main="nu=0.04",xlim=c(0,12),xlab=xxlab1,las=2)
par(pop)
...
SuboutputNu0.04Ret<-subset(outputNu0.04,RetNum>0)
tabwNu0.04Ret<-table(SuboutputNu0.04Ret[,c("NotLevel.1","CombHybP")])
