...
hyblam_path <-"/home/carlos/hybrid-Lambda-0.6.2-beta/src"#fixed
setwd("~/Desktop/Borrar/scripts")
...
#Only select network, avoid trees
out<-c()
...
#convert networks in hybrid lambda format using julia
setwd("~/Desktop/Borrar/scripts") #Go to the path of scripts and run julia code to 
ju_script <- paste("julia Net_HybridLamb_Format2.jl", temp_r_net, temp_ju_net, temp_hybla_net)
system(ju_script)



#Test of Ultrametricity

#Copy networks in Hybrid lambda format to Hybrid lambda/src to run Hybrid lambda
...
#Get Gene Trees using Hybrid lambda (../src/hybrid-Lambda -spcu ../src/JuliaHibrid2 -num 500 -seed 2 -o example3)
...
for( i in 1:length(ultrametric_networks)){
	...
	date1 <- gsub(" ", "-", Sys.time(), fixed = TRUE)
	date2 <- gsub(":", "-", date1, fixed = TRUE)
	nam_path_out_Ni <- paste(Net,"numbsim1-", numbsim1, "_ntips-", ntips,"_lambda-" ,lambda,"_mu-", mu,"_nu-", nu,"_hybprops-",hybprob ,
							 "_GTnum-",num_gene_tree,"_Rseed_",genseed,"_hylaseed_",hyla_seed,"_Date-",date2,sep="")
...
#numbsim1-500_n1-8_lambda-0.9_mu-0_nu-0.09_hybprops-0.5-0.25-0.25_GTnum-500_Date-11-18-2021-09-18-19
...
nam_path_out_Ni <- paste("numbsim1-", numbsim1, "_ntips-", ntips,"_lambda-" ,lambda,"_mu-", mu,"_nu-", nu,"_hybprops-",hybprob ,"_GTnum-",num_gene_tree,"_Rseed_",genseed,"_hylaseed_",hyla_seed,"_Date-",date2,sep="")
...
str_replace(string = ultrametric_networks[1], pattern = "JulHybrLamb", replacement= "GT")

gsub(pattern="JulHybrLamb", replacement="GT", x=ultrametric_networks[1])
