...
Identiff <- function(Tree2,
    ...
    if (any(Roo %in% c("No-Identificable", NA, NULL))) {
        ...
        NodTip3 <- Tip(ED,
                       ...
                       allret = allret) == "NoTip"
        res1 <-
            sum(c(NodTip1, NodTip2, NodTip3), na.rm = TRUE)#using (+) whit NA or NULL error will be apear, but if we use sum(,na.rm = TRUE), we omith that.7
        #res1<-NodTip1+NodTip2+NodTip3
        if (res1 >= 2) {
...
SibCross <- function(Tree2) {
    ...
    if (nrow(RetRvCiclChe) == 0) {
        ...
        if (nrow(RetEv) == 0) {
            res <-
                list(infor = c(
                    rep("not level-1", nrow(RetNoLev1)),
                    rep("RetCiclicI", nrow(RetClicli))
                ),
                ret = rbind(RetClicli, RetNoLev1))
        } else{
            outRet <- c()
            ...
            for (h in 1:nrow(RetEv)) {
                ...
                Or1whitoutRet <-
                    AssJupRet(ED, Hib1, allret)#Find the father omiting the reticulations
                Or2whitoutRet <-
                    AssJupRet(ED, Hib2, allret)#Find the father omiting the reticulations
                ...
                if (Or1whitoutRet == Or2whitoutRet) {
                    father <- Orig1
                    RootTest <-
                        AssJupRet(ED, Orig1, allret)#Go direction to the root avoiding the reticulation nodes.
...
NetDescription <- function(bdNS) {
    ...
    for (k in 1:length(bdNS)) {
        ...
        #PorExtin<-(TipNum-10)/TipNum#Proportions of extintct taxa
        NodeNum <-
            Tree2$Nnode#Number of internal Nodes# Maybe we can also divide it for TipNum
        LT2 <- ltt(Tree2, plot = FALSE)
        Tim <- LT2$times
        TimeTree <- max(Tim)#Time to reach 10 taxa
        #extracting information about the Reticulations
        ...
        if (RetNum >= 1) {
            ...
            TimeStartHibr <-
                apply(Ret1, 1, max) / TimeTree#Time when birth the hibirds, we can divide it for the time to reach 10 taxa (TimeTree)
            TSHmean <- mean(TimeStartHibr)# Average time of birth of hybrids
            TSHmin <- min(TimeStartHibr)# minimum time of birth of hybrids
            TSHmax <- max(TimeStartHibr)# maximun time of birth of hybrids
        } else{
